...
import com.fossgalaxy.game.AppEvolverplus;
...
public class AppMetrics extends AppEvolverplus {
    ...
    public static void main(String[] args) {
        ...
        GameDef def = io.loadGameDef("SaltedFish.json");
        ...
        //AppEvolver evo = new AppMetrics(io, settings, ai, "map_larg.json");
        AppEvolverplus evo = new AppMetrics(io, settings, ai, "map_larg.json");
...
//        evo.addParameter(new EntityProp("red_knight", "defRanged", 0, 10, 1));
//
//        //tweak the blue archer's attack between 0 and 10 in steps of 2
//        evo.addParameter(new EntityProp("blue_archer", "atkRanged", 0, 10, 1));

        evo.addParameter(new EntityProp("blue_solider","movement",1,5,1));
        evo.addParameter(new EntityProp("red_solider","movement",1,5,1));
        /*
        END PARAMETERS
         */
...
//        System.out.println(best.getEntityType("blue_archer").getProperty("atkRanged"));
//        System.out.println(best.getEntityType("red_knight").getProperty("defRanged"));

        System.out.println(best.getEntityType("blue_solider").getProperty("movement"));
        System.out.println(best.getEntityType("red_solider").getProperty("movement"));
    ...
    public Double evaluate(GameSettings settings) {
        ...
        String stageAgent = "SFB"; // the GA might be better here, but we don't have that kind of time...
        List<String> oppAgents = Arrays.asList("SFR");
...
//        int attack = settings.getEntityType("blue_archer").getProperty("atkRanged");
        ...
        int red_soliderMovoment = settings.getEntityType("red_solider").getProperty("movement");
        int blue_soliderMovoment = settings.getEntityType("blue_solider").getProperty("movement");
        ...
        for (int i=0; i<10; i++) {
            ...
            Controller[] controllers = new Controller[] {
                    ai.buildAI("SFB", settings),
                    ai.buildAI("SFR", settings)
            };
...
//                out.printf("%d,%d,%d,%d,%s%n", 1, i, attack, defence, String.join(",", p2Entities));
                out.printf("%d,%d,%d,%d,%s%n", 1, i, red_soliderMovoment,blue_soliderMovoment, String.join(",", p2Entities));
            }
